...
<title>Insert title here</title>
	...
	<script type="text/javascript">
		...
		function check_id() {
			...
			id_pattern = /^[0-9a-z]+$/; // id 정규표현식
			if(!id_pattern.test(id)){
				alert("아이디 입력 조건을 다시 한번 확인해 주세요.");
				return;
			}
		...
		function send( f ) {
			...
			var agree_service_check = document.getElementById("agree_service_check").checked;
			var agree_privacy_check = document.getElementById("agree_privacy_check").checked;
			...
			if( !agree_service_check ){
				alert("이용약관에 동의하세요.");
				return;
			}

			if( !agree_privacy_check ){
		...
		function checkAgreeAll() {
			var AgreeAllChecked = document.getElementById("AgreeAllChecked").checked;
			
			var agree_service_check = document.getElementById("agree_service_check");
			var agree_privacy_check = document.getElementById("agree_privacy_check");
			var is_sms = document.getElementById("is_sms");
			var is_news_mail = document.getElementById("is_news_mail");
			
			if( AgreeAllChecked ){
				agree_service_check.checked = true;
				agree_privacy_check.checked = true;
				is_sms.checked = true;
				is_news_mail.checked = true;
			} else {
				agree_service_check.checked = false;
				agree_privacy_check.checked = false;
				is_sms.checked = false;
				is_news_mail.checked = false;
			}
		}

		function checkAgree() {
			var AgreeAllChecked = document.getElementById("AgreeAllChecked");
			
			var agree_service_check = document.getElementById("agree_service_check").checked;
			var agree_privacy_check = document.getElementById("agree_privacy_check").checked;
			var is_sms = document.getElementById("is_sms").checked;
			var is_news_mail = document.getElementById("is_news_mail").checked;
			
			if( !agree_service_check || !agree_privacy_check || !is_sms || !is_news_mail ){
				AgreeAllChecked.checked = false;
			} else if( agree_service_check && agree_privacy_check && is_sms && is_news_mail ){
				AgreeAllChecked.checked = true;
			}
		}
	...
	<div id="main">
		...
		<form action="user_insert.do" method="post">
			...
			<div class="AllChecked">
		        <p>
		        	<span class="chk">
		        		<input type="checkbox" id="AgreeAllChecked" onchange="checkAgreeAll();">
		        	</span>
		        	<label for="AgreeAllChecked">
		    ...
		    <div class="agreeArea">
		        ...
		        <div class="content">
		            <p>
		            	제1조(목적)<br>
		            	이 약관은 KoreaShop이 운영하는 KoreaShop(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리․의무 및 책임사항을 규정함을 목적으로 합니다.<br>※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다.」
		            </p>
		            <p>제2조(정의)<br>
		            	① “몰”이란 KoreaShop이 재화 또는 용역(이하 “재화 등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터 등 정보통신설비를 이용하여 재화 등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.<br>
		            ...
		            <p>제3조 (약관 등의 명시와 설명 및 개정) <br>
		            	...
		            	② “몰은 이용자가 약관에 동의하기에 앞서 약관에 정하여져 있는 내용 중 청약철회․배송책임․환불조건 등과 같은 중요한 내용을 이용자가 이해할 수 있도록 별도의 공지사항을 제공하여야 합니다.<br>
	            	...
	            	<p>③ “몰”이 회원 자격을 제한․정지 시킨 후, 동일한 행위가 2회 이상 반복되거나 30일 이내에 그 사유가 시정되지 아니하는 경우 “몰”은 회원자격을 상실시킬 수 있습니다.<br>④ “몰”이 회원자격을 상실시키는 경우에는 회원등록을 말소합니다. 이 경우 회원에게 이를 통지하고, 회원등록 말소 전에 최소한 30일 이상의 기간을 정하여 소명할 기회를 부여합니다.</p>
	            	...
	            	<p>제23조(분쟁해결)<br>① “몰”은 이용자가 제기하는 정당한 의견이나 불만을 반영하고 그 피해를 보상처리하기 위하여 피해보상처리기구를 설치․운영합니다.<br>② “몰”은 이용자로부터 제출되는 불만사항 및 의견은 우선적으로 그 사항을 처리합니다. 다만, 신속한 처리가 곤란한 경우에는 이용자에게 그 사유와 처리일정을 즉시 통보해 드립니다.<br>③ “몰”과 이용자 간에 발생한 전자상거래 분쟁과 관련하여 이용자의 피해구제신청이 있는 경우에는 공정거래위원회 또는 시•도지사가 의뢰하는 분쟁조정기관의 조정에 따를 수 있습니다.
	            	</p>
	            	<p>제24조(재판권 및 준거법)<br>
		            ...
		            <p>제25조(적립금)<br>
		            	1."몰"의 재화 등을 구입하거나 결제수단으로 구입하는 회원에게 운영정책에 따라 적립금을 부여할 수 있습니다.<br>
		            	2.회원은 재화등 구매 및 배송 후 쇼핑몰 사이트 내의 적립금 확인 절차를 통하여 구매시 약정된 적립금을 부여 받을 수 있습니다.<br>
		            	3."몰"은 적립금 부여를 안내한 재화 등의 판매가에 대하여 공지사항으로 게재한 비율만큼 적립금을 부여합니다.<br>
		            	4.적립된 포인트(적립금)은 결제한 아이디의 회원 본인만 사용가능하며, 타인 및 다른 아이디로 양도할 수 없습니다.<br>
		            	5.회원이 구매한 상품에 대하여 품절 및 회원의 요청 등으로 취소 및 반품 상황이 발생하였을 경우, 이미 적립된 적립금을 사용한 때에는 개인의 기적립 적립금에서 사용분 만큼 차감됩니다.<br>
		            	이미 적립된 적립금 잔여분이 없을 경우에는 사용분 금액에 대하여 별도 대금을 지급하여야 합니다.<br>
		            	6.회원에게 적립된 포인트가 부정한 방법 또는 개인정보 도용으로 인해 적립된 사실이 인정되거나 회사 전산 프로그램 오류 등의 사유에 의하여 "몰"은 해당 포인트를 별도의 통보없이 "몰"이 정정 또는 삭감할 수 있습니다.<br>
		            	7.적립금 사용 가능 조건 및 소멸에 대한 내용이 별도로 공지된 경우(그 기간을 사용기한으로 정함) 또한 적립금의 유효기간은 적립금 지급 시점을 기준으로 12개월이며, 유효기간 내에 사용되지 않은 적립금은 매년 2월 1일 자동 소멸됩니다.
		            </p>
	            	<p>제26조(적립금과 예치금)<br>
		            	1.적립금과 예치금은 "몰"내에서 현금처럼 사용되는 전자화폐이며, 상품 구매시에만 이용할 수 있습니다.<br>
		            	2.예치금은 회원에 한하여 제공되는 서비스이며 신용카드,휴대폰결제,현금을 이용하여 지불할 수 있습니다.<br>
		            	3.적립금은 회원에 한하여 제공되는 서비스입니다.<br>
		            	4.적립금과 예치금은 타 결제수단(신용카드,휴대폰결제,현금)과 함께 사용될 수 있습니다.<br>
		            	5."몰"은 각종 할인이벤트 또는 제휴이벤트 등의 특정기간에는 적립금 또는 예치금의 신청 및 이용을 제한할 수 있습니다.<br>
		            	6.회원이 상품구매나 이벤트 등을 통하여 회사로부터 무상 제공받은 적립금의 경우 "몰"에서 정한 기한에 한해서만 이용 가능합니다.<br>
		            	7.유효기간:"몰"로부터 제공받은 적립금의 경우 "몰"이 별도로 유효기간을 정한 때에는 이에 따릅니다.<br>
		            	  유효기간이 경과한 적립금은 자동적으로 소멸됩니다.<br>
		            	8.적립금 소멸일 기준은 1년이상 경과된 적립금에 대해 "몰"은 해당 회원의 적립된 적립금을 회수하게 됩니다.<br>
		            	또한 적립금의 유효기간은 적립금 지급 시점을 기준으로 12개월이며, 유효기간 내에 사용되지 않은 적립금은 매년 2월 1일 자동 소멸됩니다.<br>
		            	 1)삭제 대상 회원:제26조 8번 항목에 작성된 삭제 조건에 부합하는 회원<br>
		            	 2)해당조건에 부합하는 회원의 경우, 해당 기간이 지난 후 자동적으로 적립금 소멸됨.</p>
	            	<p>제27조(휴면회원)<br>
		            	"몰"은 최근 1년간 구매이력이 없는 경우, 그로 인하여 최종 적립금 발생일로부터 1년간 적립금 추가누적이 없는 경우 휴면회원으로 간주합니다.<br>
		            	해당 회원의 적립금은 자동으로 소멸됩니다.<br>
		            	"몰"은 휴면회원의 개인정보 분리 저장 및 관리 30일 전에 해당 회원에게 회원자격 상실에 대한 안내문을 메일 등의 수단으로 통지하며, 안내문에서 정한 기한 내에 답변이 없을 경우 회원자격을 상실시킬 수 있습니다.<br>
		            	휴면회원의 경우 1년간 이용실적이 없는 경우를 기준으로 하며, 적립금 소멸기준은 이용약관 제26조를 기준으로 모든 회원 동일하게 적용되어 소멸 될 수 있습니다.
		            </p>
	            	<p>변경이력<br>
	            		이 약관은 2018년 12월 20일부터 시행하며 종전의 약관은 이 약관으로 대체 합니다.
	            	</p>
	            	<p>#카카오톡 알림톡 시행에 관한 내용</p>
		            <p>“ KoreaShop ”는 회원가입, 주문안내, 배송안내 등 비광고성 정보를 카카오톡 알림톡으로 알려드리며, 만약 알림톡 수신이 불가능하거나 수신 차단하신 경우에는 일반 문자메시지로 보내드립니다. 카카오톡 알림톡을 통해 안내되는 정보를 와이파이가 아닌 이동통신망으로 이용할 경우, 알림톡 수신 중 데이터 요금이 발생할 수 있습니다. 카카오톡을 통해 발송되는 알림톡 수신을 원치 않으실 경우 반드시 알림톡을 차단하여 주시기 바랍니다.</p>
		            <p>#카카오톡 상담톡 시행에 관한 내용</p>
	            	<p>
		            	<span style="font-size: 9pt;">“ KoreaShop ”는 상담업무를 카카오톡 상담톡으로 진행하며, 만약 카카오톡을 통해 안내되는 상담 내용을 Wi-Fi 나 PC가 아닌 이동통신망으로 이용할 경우, 데이터 요금이 발생할 수 있습니다. 카카오톡을 통해 상담을 원치 않으실 경우 고객센터, 게시판 문의를 이용해 주시기 바랍니다.</span>
		           	</p>																																								
		        </div>
		        <p class="check">
			        <span>이용약관에 동의하십니까?</span> 
			        <input id="agree_service_check" value="1" type="checkbox" onchange="checkAgree();">
		    ...
		    <div class="agreeArea">
		        ...
		        <div class="content">
		            ...
		            <p>가. 회사 내부 방침에 의한 정보 보유 사유</p><p>o 부정거래 방지 및 쇼핑몰 운영방침에 따른 보관 : 5년</p>
	        	...
	        	<p class="check">
		        	...
		        	<input id="agree_privacy_check" value="1" type="checkbox" onchange="checkAgree();">
			...
			<div class="agreeArea ">
		        ...
		        <ul class="check">
					<li>
		                ...
		                <label for="is_sms"><input id="is_sms" value="T" type="checkbox" onchange="checkAgree();">동의함</label>            
	                </li>
		            <li>
		                <span>이메일 수신을 동의하십니까?</span> 
		                <label for="is_news_mail"><input id="is_news_mail" class="chk" value="T" type="checkbox" onchange="checkAgree();">동의함</label>          
	                </li>
		        </ul>
			</div>	
